  ...
  if (H_prior_.rows() > 0)
  {
    ...
    for (auto vertex : verticies_)
    {
      if (IsPoseVertex(vertex.second) && vertex.second->IsFixed())
      {
        ...
        //                std::cout << " fixed prior, set the Hprior and bprior
...
void Problem::SolveLinearSystem()
{
  ...
  else
  {
    ...
    // std::cout << " Linear Solver Time Cost: " << t_linearsolver.toc() <<
    // std::endl;
...
void Problem::UpdateStates()
{
  ...
  if (err_prior_.rows() > 0)
  {
    ...
    //        std::cout << "                : "<< b_prior_.norm()<<" "
    //        <<err_prior_.norm()<< std::endl; std::cout << "     delta_x_ ex:
...
void Problem::ComputeLambdaInitLM()
{
  ...
  //        std::cout << "currentLamba_: "<<maxDiagonal<<"
    ...
    const std::vector<std::shared_ptr<Vertex>> margVertexs, int pose_dim)
{
  ...
  // std::cout << "my marg b prior: " <<b_prior_.rows()<<" norm: "<<
  // b_prior_.norm() << std::endl; std::cout << "    error prior: "
